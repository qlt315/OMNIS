...
testing_quant_with_noise = True
new_standard = True
new_standard_ft = 2 # 1, 2, None #should only be not None when training stage is set to 2
...
if training_stage == '1' or train_teacher or new_standard_ft == '1':
...
detector_head = MaskRCNN_wrapper_HEAD(bottleneck_channel=channels, is_training=(training_stage=='1'), entropy_split=entropy_model, new_standard=new_standard).to(device)#torchvision.models.detection.fasterrcnn_resnet50_fpn_v2(weights="COCO_V1").to(device)#
...
if not train_teacher:
    ...
    if (training_stage == '2' and entropy_model):
        if use_nu:
            ...
            detector_head.load_state_dict(torch.load('models/entrop/MaskRCNN_head-S1-'+str(channels)+'CH-L0500.pth'))
            detector_tail.load_state_dict(torch.load('models/entrop/MaskRCNN_tail-S1-'+str(channels)+'CH-L0500.pth'))
    ...
    elif testing_quant_with_noise and not new_standard:
        detector_head.load_state_dict(torch.load('models/standard/MaskRCNN_head-S2-'+str(channels)+'CH.pth'), strict=False)
        detector_tail.load_state_dict(torch.load('models/standard/MaskRCNN_tail-S2-'+str(channels)+'CH.pth'))
    elif new_standard_ft == '2':
        # TODO: Retrained models and did not implement the enchanced training yet
        # detector_head.load_state_dict(torch.load('models/new_stand/stand/MaskRCNN_head-S2ft1-'+str(channels)+'CH.pth'), strict=False)
        # detector_tail.load_state_dict(torch.load('models/new_stand/stand/MaskRCNN_tail-S2ft1-'+str(channels)+'CH.pth'))
        detector_head.load_state_dict(torch.load('models/new_stand/stand/MaskRCNN_head-S2-'+str(channels)+'CH.pth'), strict=False)
        detector_tail.load_state_dict(torch.load('models/new_stand/stand/MaskRCNN_tail-S2-'+str(channels)+'CH.pth'))
    elif new_standard and training_stage == '2':
        detector_head.load_state_dict(torch.load('models/new_stand/stand/MaskRCNN_head-S1-'+str(channels)+'CH.pth'), strict=False)
        detector_tail.load_state_dict(torch.load('models/new_stand/stand/MaskRCNN_tail-S1-'+str(channels)+'CH.pth'))
        # if new_standard_ft == '1':
        #     detector_tail.backbone_tail.gt_decoder = deepcopy(detector_tail.backbone_tail.decoder)
    elif training_stage == '2' or new_standard:
        detector_head.load_state_dict(torch.load('models/standard/MaskRCNN_head-S1-'+str(channels)+'CH.pth'), strict=False)
...
def training_loop(model_head, model_tail, learning_rate, train_dataloader, n_epochs, val_loader, batch_size):
    ...
    if train_teacher:
        ...
        if training_stage == '1':
            ...
            # optimizer = torch.optim.SGD((list(model_tail.backbone_tail.decoder.conv3.parameters()) + list(model_tail.backbone_tail.decoder.bn3.parameters()) +
            #                              list(model_tail.backbone_tail.decoder.conv4.parameters()) + list(model_tail.backbone_tail.decoder.bn4.parameters())), lr=learning_rate, momentum=0.5, weight_decay=1e-5)
    ...
    for epoch in range(n_epochs):#tqdm(range(n_epochs)):#
            ...
            if training_stage == '1':
                ...
                if new_standard:
                    ...
                    if new_standard_ft == '1':
                        ...
                        # model_tail.backbone_tail.gt_decoder.eval()
                        # model_tail.backbone_tail.decoder.conv3.train()
                        # model_tail.backbone_tail.decoder.bn3.train()
                        # model_tail.backbone_tail.decoder.conv4.train()
                        # model_tail.backbone_tail.decoder.bn4.train()
                        model_tail.backbone_tail.use_gt = False
        ...
        if not testing and not first_loop[0]:
                ...
                if train_teacher:
                    ...
                    if training_stage == '1':
                        if entropy_model:
                            ...
                            bpp_loss = 0.03 * model_head.backbone_head.encoder.quant_thing.bpp #0.15, 0.075, 0.0375
                            # self_target_loss = 10*(mse_loss(model_head.backbone_head.encoder.quant_thing.self_target, model_head.backbone_head.encoder.self_target))
                            self_target_loss = torch.tensor(0).cuda()
                            # print(bpp_loss.item())
                            aux_loss = 0
                        else:
                            bpp_loss = 0
                        ...
                        instace_mse_loss = 10*(mse_loss(model_tail.backbone_tail.decoder_out, teacher.l1_out)) + self_target_loss
                ...
                if i != 0 and i % 10 == 0 and printed == False:
                    ...
                    if training_stage == '1' and not train_teacher:
                        if entropy_model:
                            ...
                            print(f'mse_loss: %.5f | self_target_loss: %.5f | bpp_loss: %.5f | recon_loss %.5f |' %(instace_mse_loss.item(), self_target_loss.item(), bpp_loss.item(), recon_loss.item()))
            ...
            if train_teacher:
                ...
                if training_stage == '1':
                    if entropy_model:
                        torch.save(model_head.state_dict(), 'models/rsud_models/MaskRCNN_head-S1-'+str(channels)+'CH-L0500.pth')
                        torch.save(model_tail.state_dict(), 'models/rsud_models/MaskRCNN_tail-S1-'+str(channels)+'CH-L0500.pth')
                    elif new_standard:
                        torch.save(model_head.state_dict(), 'models/new_stand/stand/MaskRCNN_head-S1-'+str(channels)+'CH.pth')
                        torch.save(model_tail.state_dict(), 'models/new_stand/stand/MaskRCNN_tail-S1-'+str(channels)+'CH.pth')
                    else:
                        torch.save(model_head.state_dict(), 'models/standard/MaskRCNN_head-S1-'+str(channels)+'CH.pth')
                        torch.save(model_tail.state_dict(), 'models/standard/MaskRCNN_tail-S1-'+str(channels)+'CH.pth')
                elif training_stage == '2':
                    if entropy_model:
                        torch.save(model_head.state_dict(), 'models/rsud_models/MaskRCNN_head-S2-'+str(channels)+'CH-L0500.pth')
                        torch.save(model_tail.state_dict(), 'models/rsud_models/MaskRCNN_tail-S2-'+str(channels)+'CH-L0500.pth')
                    elif new_standard_ft == '1':
                        torch.save(model_head.state_dict(), 'models/new_stand/stand/MaskRCNN_head-S2ft1-'+str(channels)+'CH.pth')
                        torch.save(model_tail.state_dict(), 'models/new_stand/stand/MaskRCNN_tail-S2ft1-'+str(channels)+'CH.pth')
                    elif new_standard_ft == '2':
                        torch.save(model_head.state_dict(), 'models/new_stand/stand/MaskRCNN_head-S2ft2-'+str(channels)+'CH.pth')
                        torch.save(model_tail.state_dict(), 'models/new_stand/stand/MaskRCNN_tail-S2ft2-'+str(channels)+'CH.pth')
                    elif new_standard:
                        torch.save(model_head.state_dict(), 'models/new_stand/stand/MaskRCNN_head-S2-'+str(channels)+'CH.pth')
                        torch.save(model_tail.state_dict(), 'models/new_stand/stand/MaskRCNN_tail-S2-'+str(channels)+'CH.pth')
                    else:
                        torch.save(model_head.state_dict(), 'models/standard/MaskRCNN_head-S2-'+str(channels)+'CH.pth')
                        torch.save(model_tail.state_dict(), 'models/standard/MaskRCNN_tail-S2-'+str(channels)+'CH.pth')
            print('saving model')
        ...
        if train_teacher:
            ...
            if new_standard:
                ...
                if new_standard_ft is not None and testing_quant_with_noise:
                    ...
                    model_head.backbone_head.encoder.ber = bers[0] #0,3,5,7,10
                    print('its true')
            ...
            with torch.no_grad():
                ...
                for data in val_loader:#tqdm(val_loader):#
                    ...
                    if (training_stage == '1' or training_stage == '2') and not test_teacher:
                        ...
                        elif testing_quant_with_noise and training_stage == '2' and not new_standard:
                            features_from_head, size_holder = process_raw(features_from_head, bers[10])
