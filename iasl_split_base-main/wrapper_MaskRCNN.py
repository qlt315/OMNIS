...
class MaskRCNN_wrapper(nn.Module):
    def __init__(self):
        super().__init__()

        model = torchvision.models.detection.maskrcnn_resnet50_fpn_v2(weights="COCO_V1", pretrained=True)
...
class MaskRCNN_wrapper_HEAD(nn.Module):
    def __init__(self, bottleneck_channel=6, is_training=True, entropy_split=True, new_standard=False): #we only need from_old if using model before we created new ROIHeads
        super().__init__()
...
class MaskRCNN_wrapper_TAIL(nn.Module):
