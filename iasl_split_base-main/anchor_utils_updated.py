...
class AnchorGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.sizes = anchor.sizes
        self.aspect_ratios = anchor.aspect_ratios
        self.cell_anchors = [
            self.generate_anchors(size, aspect_ratio) for size, aspect_ratio in zip(self.sizes, self.aspect_ratios)
        ]
    ...
    def forward(self, image_size: torch.Tensor.size, image_sizes: List[Tuple[int,int]], feature_maps: List[Tensor]) -> List[Tensor]:
        grid_sizes = [feature_map.shape[-2:] for feature_map in feature_maps]
        # image_size = image_list.tensors.shape[-2:]
        dtype, device = feature_maps[0].dtype, feature_maps[0].device
        # print(image_size, grid_sizes)
        strides = [
            [
