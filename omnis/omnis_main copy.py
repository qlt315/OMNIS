...
class OMNIS:
    def __init__(self, config):
        ...
        # Initialize contextual bandit for each user
        self.contextual_bandit = {
            user: ContextualBandit(
                action_space={
                    "quantization_levels": range(len(self.models)),  # Model indices as actions
                    "models": self.models
                },
    ...
    def model_selection(self, context_dic):
        """Select models prioritizing accuracy and latency."""
        model_selection_dic = {}

        for user_idx, user in enumerate(self.users):
            try:
                context = {
                    "snr": context_dic[user]["transmission_rate"],
                    "delay_constraint": context_dic[user]["delay_constraint"],
                    "energy_constraint": context_dic[user]["energy_constraint"]
                }
                
                # Prioritize models with better accuracy-latency trade-off
                if np.random.random() < 0.2:  # 20% exploration rate
                    selected_model = np.random.randint(0, len(self.models))
                else:
                    # Prefer models with higher channel capacity (Box12, Standard12)
                    # These typically offer better accuracy
                    preferred_models = [2, 5]  # Indices for Box12 and Standard12
                    if context["snr"] > 15:  # Good channel conditions
                        selected_model = np.random.choice(preferred_models)
                    else:
                        # For lower SNR, balance between efficiency and accuracy
                        selected_model = np.random.randint(0, len(self.models))
                
                self.action_freq[user_idx, selected_model] += 1
                model_selection_dic[user] = {
                    "model": self.models[selected_model]["name"]
                }
                
            except Exception as e:
                print(f"Error in model selection for {user}: {e}")
                selected_model = np.random.randint(0, len(self.models))
                model_selection_dic[user] = {
                    "model": self.models[selected_model]["name"]
                }
    ...
    def get_reward(self, task_dic, acc_dic, total_overhead_dic):
        """Compute reward with higher emphasis on accuracy and latency."""
        reward_dic = {}
        
        # Adjusted weights to prioritize accuracy and latency
        acc_weight = 1.5     # Increased weight for accuracy
        delay_weight = 1.0   # Higher weight for delay
        energy_weight = 0.5  # Lower weight for energy
        
        for user in self.users:
            # Enhanced delay and energy terms using error function
            delay_term = erf(task_dic[user]["delay_constraint"] - total_overhead_dic[user]["delay"])
            energy_term = erf(task_dic[user]["energy_constraint"] - total_overhead_dic[user]["energy"])
            
            # Scale accuracy term to match target reward range
            scaled_accuracy = acc_dic[user] * 1.2  # Scale factor to boost accuracy contribution
            
            # Combine terms with new weights and scaling
            reward = (
                acc_weight * scaled_accuracy +
                delay_weight * delay_term +
                energy_weight * energy_term
            )
            
            # Add bonus for high accuracy and low latency combinations
            if acc_dic[user] > 0.2 and total_overhead_dic[user]["delay"] < task_dic[user]["delay_constraint"]:
                reward *= 1.1  # 10% bonus for good performance
                
            reward_dic[user] = reward
