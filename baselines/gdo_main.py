...
class GDO:
    ...
    def greedy_model_selection(self, model_selection_dic, snr_dic, ldpc_rate_dic, acc_dic):
        ...
        min_acc_user = min(acc_dic, key=acc_dic.get)  # User with the minimum accuracy
        max_acc_user = max(acc_dic, key=acc_dic.get)  # User with the maximum accuracy


        # Get the current model index for the user with minimum accuracy
        min_acc_user_model_index = next(
            (index for index, model in enumerate(self.models)
             if model['name'] == model_selection_dic[min_acc_user]),
            None
        )
        # Get the current model index for the user with maximum accuracy
        max_acc_user_model_index = next(
            (index for index, model in enumerate(self.models)
             if model['name'] == model_selection_dic[max_acc_user]),
            None
        )
        temp_model_selection_dic = model_selection_dic
        # Modify the model for the user with minimum accuracy (index-1)
        if min_acc_user_model_index is not None and min_acc_user_model_index > 0:
            # Change the model to the previous model (index-1)
            new_model_index = min_acc_user_model_index - 1
            temp_model_selection_dic[min_acc_user] = self.models[new_model_index]

        # Modify the model for the user with maximum accuracy (index+1)
        if max_acc_user_model_index is not None and max_acc_user_model_index < len(self.models) - 1:
            # Change the model to the next model (index+1)
            new_model_index = max_acc_user_model_index + 1
            temp_model_selection_dic[max_acc_user] = self.models[new_model_index]

        # Calculate the new accuracy after modification
        temp_acc_dic = self.get_accuracy(snr_dic, ldpc_rate_dic, temp_model_selection_dic)


        # Compare the new minimum accuracy with the original minimum accuracy
        if min(temp_acc_dic.values()) < min(acc_dic.values()):
            # If the new accuracy is better, apply the changes
            model_selection_dic = temp_model_selection_dic

        # Return the updated model selection dictionary
...
if __name__ == "__main__":
    ...
    gdo = GDO(config)
    gdo.simulation()
    print("aver info:", gdo.average_metrics)
    print("std info:", gdo.std_metrics)
    print("action freq info:", gdo.action_freq)
    gdo.show_convergence()
