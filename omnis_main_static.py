...
class OMNIS:
    def __init__(self):
        ...
        self.gp_models = {}
        self.gp_data = {user: {'x': [], 'y': []} for user in self.users}  # Store training data
        kernel = C(1.0, (1e-4, 1e1)) * Matern(length_scale=1.0, nu= 3 / 2, length_scale_bounds="fixed")
        for user in self.users:
            self.gp_models[user] = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10)
    ...
    def model_selection(self, context_dic):
        ...
        for user in self.users:
            # Convert the user context to a NumPy array
            context_m = np.array(list(context_dic[user].values())).reshape(1, -1)

            model_rewards = []
            for model in self.models:
                model_feature = context_m.reshape(1, -1)
                # Use Gaussian Process (GP) to predict the expected reward and uncertainty
                mean, std = self.gp_models[user].predict(model_feature, return_std=True)

                # Compute Upper Confidence Bound (UCB)
                ucb_value = mean + 0.6 * std

                # Store the model name, UCB value, and feature vector
                model_rewards.append((model['name'], ucb_value, model_feature))

            # Select the model with the highest UCB value
            best_model = max(model_rewards, key=lambda x: x[1])

            # Store the selected model for the user
            model_selection_dic[user] = {
                "model_name": best_model[0],  # Best model name
                "ucb_value": best_model[1],  # UCB value
                "model_feature": best_model[2]  # Feature vector used for selection
            }

        return model_selection_dic  # Return the best model for all users

    def update_gp(self, context_dic, reward_dic):
        """Update the GP model with new observations."""
        for user in self.users:
            # Convert the context values from context_dic to a 1 x n 2D array
            context_m = np.array(list(context_dic[user].values())).reshape(1, -1)
            self.gp_data[user]['x'].append(context_m)
            self.gp_data[user]['y'].append(reward_dic[user])

            # Concatenate all 2D arrays into a single 2D array
            x_train = np.concatenate(self.gp_data[user]['x'], axis=0)  # Combine all context observations into one array
            y_train = np.array(self.gp_data[user]['y'])

            # Fit the Gaussian Process model
            self.gp_models[user].fit(x_train, y_train)

    def get_reward(self, task_dic, acc_dic, total_overhead_dic):
        """Compute the reward of MDs based on accuracy and penalty terms."""
        reward_dic = {}
        for user in self.users:
            reward_dic[user] = (acc_dic[user] + task_dic[user]["delay_weight"] * erf(task_dic[user]["delay_constraint"] - total_overhead_dic[user]["delay"])
                                + task_dic[user]["energy_weight"] * erf(task_dic[user]["energy_constraint"] - total_overhead_dic[user]["energy"]))
    ...
    def allocate_bandwidth(self,task_dic, model_selection_dic, trans_rate_dic, ldpc_rate_dic):
        ...
        d_prime_dic = {}
        for user in self.users:
            chosen_model_m = model_selection_dic[user]["model_name"]
    ...
    #     c_m = cp.Variable(self.user_num, integer=True)
    #
    #     # Constraints
    #     constraints = [0 <= c_m, c_m <= self.es_params['cores'], cp.sum(c_m) <= self.es_params['cores']]
    #
    ...
    #
    #     head_flops_vector = np.array([self.head_flops[model_selection_dic[user]["model_name"]] for user in self.users])
    ...
    #     first_term = omega_m_t_vector * head_flops_vector @ cp.inv_pos(c_m)  /  (freq * flops_per_cycle)
    #     second_term = omega_m_e_vector * power_coeff * (freq ** 2) @ cp.inv_pos(c_m) / flops_per_cycle
    #     total_sum = cp.sum(first_term + second_term)
    #
    #
    ...
    #         gpu_allocation_dict[user] = c_m.value[idx]
    #     print(gpu_allocation_dict)
    ...
    def coding_rate_selection(self, task_dic, snr_dic, trans_rate_dic, model_selection_dic,
        ...
        for user in self.users:
            ...
            for coding_rate in self.available_coding_rate:
                ...
                trans_overhead_dic = self.get_trans_overhead(trans_rate_dic, model_selection_dic, bandwidth_allocation_dic,
                                                         temp_ldpc_rate_dic)

                # Calculate total delay and energy
                total_delay = (local_overhead_dic[user]['delay'] + edge_overhead_dic[user]['delay'] + trans_overhead_dic[user][
                    'delay'])
                total_energy = (
                            local_overhead_dic[user]['energy'] + edge_overhead_dic[user]['energy'] + trans_overhead_dic[user][
                        'energy'])
    ...
    def get_accuracy(self,snr_dic, ldpc_rate_dic, model_selection_dic):
        ...
        for user in self.users:
            chosen_model_m = model_selection_dic[user]["model_name"]  # Get the selected model for this user
            df_filtered = self.acc_data[(self.acc_data["Model"] == chosen_model_m) & (self.acc_data["Coding Rate"] == ldpc_rate_dic[user])]
    ...
    def get_local_overhead(self, model_selection_dic):
        ...
        for user in self.users:
            ...
            # Update model_selection_dic with the user's model selection details (already in format)
            model_selection_dic[user]["local_delay"] = local_delay
            model_selection_dic[user]["local_energy"] = local_energy
    ...
    def get_total_overhead(self,local_overhead_dic, trans_overhead_dic, edge_overhead_dic):
        ...
        for user in self.users:
            total_delay = local_overhead_dic[user]['delay'] + trans_overhead_dic[user]['delay'] + edge_overhead_dic[user]['delay']
    ...
    def show_reward(self,reward_history):
        ...
        for user in self.rewards_history:
            ...
            print(self.rewards_history[user])
    ...
    def simulation(self):
        ...
        for t in range(self.time_slot_num):
            ...
            self.update_gp(context_dic, reward_dic)
