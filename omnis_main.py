...
class OMNIS:
    ...
    def generate_tasks(self, time_slot):
        ...
        task_dic = {
            user: {
                "delay_constraint": self.fixed_delay[user] + 0.001 * np.sin(time_slot),
                "energy_constraint": self.fixed_energy[user] + 0.1 * np.cos(time_slot),
                "energy_weight": self.fixed_energy_weight[user] + 0.05 * np.sin(time_slot),
            }
            for user in self.users
        }
