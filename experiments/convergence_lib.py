...
except ImportError:  # when imported as experiments.convergence_lib
...
def write_convergence_exports(out_dir, slide=5, user_num=None):
    """Write ``plot_data.mat`` + Python sidecars from ``perseed.csv`` + ``series/``."""
    try:
    ...
    if not rows:
        print(f"warning: no perseed.csv under {out_dir}; skip exports", flush=True)
        return None
    ...
    print(f"wrote plot exports -> {mat_path} (+ .pkl/.npz)", flush=True)
    return mat_path


# Back-compat alias
write_train_mat = write_convergence_exports
...
def run_convergence(algos, slots=300, users=10, seeds=(0, 1, 2, 3, 4),
    ...
    if write_mat:
        write_convergence_exports(out, slide=slide, user_num=users)
    if do_plot:
        try:
...
# Aliases
PYCHARM_TRAIN_ALGOS = PYCHARM_CONV_ALGOS
PYCHARM_TRAIN_SLOTS = PYCHARM_CONV_SLOTS
PYCHARM_TRAIN_USERS = PYCHARM_CONV_USERS
PYCHARM_TRAIN_SEEDS = PYCHARM_CONV_SEEDS
PYCHARM_TRAIN_OUT = PYCHARM_CONV_OUT
PYCHARM_TRAIN_PLOT = PYCHARM_CONV_PLOT
PYCHARM_TRAIN_WRITE_MAT = PYCHARM_CONV_WRITE_MAT


def cli_main(default_algos=None, argv=None):
    """Entry for ``convergence_all.py`` / ``convergence_*.py`` (PyCharm-safe)."""
    try:
        from repo_util import ensure_repo_root
    except ImportError:
    ...
    p = argparse.ArgumentParser(
        description="OMNIS convergence / online evaluation "
                    "(CSV + series + plot_data.mat/.pkl/.npz)")
    p.add_argument(
    ...
    p.add_argument("--out", default=None,
                   help="CSVs + series + plot exports output dir")
    ...
    elif PYCHARM_CONV_ALGOS is not None:
        algos = resolve_algos(
            PYCHARM_CONV_ALGOS if PYCHARM_CONV_ALGOS != "all"
            else ["all"])
    else:
        algos = resolve_algos(script_default)

    slots = args.slots if args.slots is not None else PYCHARM_CONV_SLOTS
    users = args.users if args.users is not None else PYCHARM_CONV_USERS
    seeds = tuple(args.seeds if args.seeds is not None else PYCHARM_CONV_SEEDS)
    out = args.out if args.out is not None else PYCHARM_CONV_OUT
    do_plot = bool(args.plot or PYCHARM_CONV_PLOT)
    write_mat = bool(PYCHARM_CONV_WRITE_MAT) and not args.no_mat

    run_convergence(algos, slots=slots, users=users, seeds=seeds, out=out,
